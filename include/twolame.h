...
/** Set Rawmode Switch for input.
 *
 *  \param glopts 	pointer to twolame options pointer
 *  \return			0
 */
DLL_EXPORT int twolame_set_rawmode (twolame_options *glopts, int rawmode);


/** Get Rawmode Switch input setting.
 *
 *  \param glopts 	pointer to twolame options pointer
 *  \return			the rawmode
 */
DLL_EXPORT int twolame_get_rawmode (twolame_options *glopts);
...
/** Set the samplesize of the PCM audio input.
 *
 *	Default: 16 bits
 *
 *	\param glopts			pointer to twolame options pointer
 *	\param samplesize		the samplesize in bits
 *	\return					0 if successful, 
 *							non-zero on failure
 */
DLL_EXPORT int twolame_set_samplesize(twolame_options *glopts, int samplesize);


/** Get the samplesize of the PCM audio input.
 *
 *	\param glopts	pointer to twolame options pointer
 *	\return			the output samplesize
 */
DLL_EXPORT int twolame_get_samplesize(twolame_options *glopts);


/** Set samplefloat if Float audio input.
 *
 *	Default: 0 (NOT float)
 *
 *	\param glopts			pointer to twolame options pointer
 *	\param samplefloat		1 if float
 *	\return					0 if successful, 
 *							non-zero on failure
 */
DLL_EXPORT int twolame_set_samplefloat(twolame_options *glopts, int samplefloat);


/** Get samplefloat if Float audio input.
 *
 *	\param glopts	pointer to twolame options pointer
 *	\return			0 if NOT float, 1 if float
 */
DLL_EXPORT int twolame_get_samplefloat(twolame_options *glopts);
